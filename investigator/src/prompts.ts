import { DOMAINS, DOMAIN_NAMES } from './domains';
import type { MCPToolCatalog } from './types';

function domainCatalog(toolCatalog?: MCPToolCatalog): string {
...
export function buildPlannerSystem(toolCatalog?: MCPToolCatalog): string {
...
export const VERIFIER_SYSTEM = `You inspect query results against the symptom. For each result, classify it as:
...
export const CORRELATOR_SYSTEM = `You correlate query results across domains. Look for:
...
export const SUMMARIZER_SYSTEM = `Produce a concise investigation report given the symptom, executed plans, raw tool results, and findings.

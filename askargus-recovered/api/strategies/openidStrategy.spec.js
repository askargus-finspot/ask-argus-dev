...
const { ErrorTypes } = require('askargus-data-provider');
const { findUser, createUser, updateUser } = require('~/models');
const { resolveAppConfigForUser } = require('@askargus/api');
...
jest.mock('@askargus/api', () => ({
  ...
  findOpenIDUser: jest.requireActual('@askargus/api').findOpenIDUser,
...
describe('setupOpenId', () => {
  ...
  describe('Tenant-scoped config', () => {
    ...
    it('should block login when tenant config restricts the domain', async () => {
      const { isEmailDomainAllowed } = require('@askargus/api');
      const existingUser = {

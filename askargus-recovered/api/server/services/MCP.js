...
const { logger, getTenantId } = require('@askargus/data-schemas');
const {
  Providers,
  StepTypes,
  GraphEvents,
...
const { Time, CacheKeys, Constants, isAssistantsEndpoint } = require('askargus-data-provider');
const {
 ...
 * @returns {Promise<Record<string, import('@askargus/api').ParsedServerConfig>>}
 */
...
function createRunStepEmitter({ res, runId, stepId, toolCall, index, streamId = null }) {
  return async function () {
    /** @type {import('@vediyappanm05/agents').RunStep} */
    const data = {
 ...
 * @param {AbortSignal} [params.signal]
 * @param {string | null} [params.streamId] - The stream ID for resumable mode.
 * @param {import('@askargus/api').ParsedServerConfig} [params.config]
 * @param {Record<string, Record<string, string>>} [params.userMCPAuthMap]
 * @returns { Promise<Array<typeof tool | { _call: (toolInput: Object | string) => unknown}>> } An object with `_call` method to execute the tool input.
 */
...
async function getMCPSetupData(userId, options = {}) {
  ...
  const mcpManager = getMCPManager(userId);
  /** @type {Map<string, import('@askargus/api').MCPConnection>} */
  ...
  try {
    ...
    // that require user context (e.g., those with {{ASKARGUS_USER_ID}} placeholders).
 ...
 * @param {string} userId - The user ID
 * @param {string} serverName - The server name
 * @param {import('@askargus/api').ParsedServerConfig} config - The server configuration
 * @param {Map<string, import('@askargus/api').MCPConnection>} appConnections - App-level connections
 * @param {Map<string, import('@askargus/api').MCPConnection>} userConnections - User-level connections

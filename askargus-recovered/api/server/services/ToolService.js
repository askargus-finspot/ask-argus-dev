 ...
 */
/** Native AskArgus tools that are not in the manifest */
 ...
 * @returns {Promise<{
 *   toolDefinitions?: import('@askargus/api').LCTool[];
 *   toolRegistry?: Map<string, import('@askargus/api').LCTool>;
 *   userMCPAuthMap?: Record<string, Record<string, string>>;
 *   hasDeferredTools?: boolean;
 * }>}
 */
      ...
      if (!isDomainAllowed) {
        logger.warn(
          `[Actions] Domain "${action.metadata.domain}" not in allowedDomains. ` +
            `Add it to askargus.yaml actions.allowedDomains to enable this action.`,
        );
        continue;
      }

...
} = require('@askargus/data-schemas');
const { ErrorTypes, SystemRoles, errorsToString } = require('askargus-data-provider');
const {
  math,
  isEnabled,
  checkEmailConfig,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'AskArgus',
...
const setAuthTokens = async (userId, res, _session = null) => {
  try {
    ...
    res.cookie('token_provider', 'askargus', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: shouldUseSecureCookie(),
      sameSite: 'strict',

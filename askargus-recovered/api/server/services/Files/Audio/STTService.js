...
const { genAzureEndpoint, logAxiosError } = require('@askargus/api');
const { extractEnvVariable, STTProviders } = require('askargus-data-provider');
...
class STTService {
  ...
  async getProviderSchema(req) {
    ...
    if (!sttSchema) {
      throw new Error(
        'No STT schema is set. Did you configure STT in the custom config (askargus.yaml)?',
      );
    }

...
// Mock askargus-data-provider with configurable file size limit
jest.mock('askargus-data-provider', () => {
  const actual = jest.requireActual('askargus-data-provider');
  return {
    ...actual,
...
const { FileContext } = require('askargus-data-provider');
...
// Mock axios — process.js now uses createAxiosInstance() from @askargus/api
...
const { codeServerHttpAgent, codeServerHttpsAgent } = require('@askargus/api');

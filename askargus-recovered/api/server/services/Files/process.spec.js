...
const { EToolResources, FileSources, AgentCapabilities } = require('askargus-data-provider');
const { mergeFileConfig } = require('askargus-data-provider');
...
describe('processAgentFileUpload', () => {
  ...
  describe('OCR strategy selection', () => {
    ...
    test('throws instead of falling back to parseText when document_parser fails for a document MIME type', async () => {
      ...
      const req = makeReq({ mimetype: PDF_MIME, ocrConfig: null });
      const { parseText } = require('@askargus/api');

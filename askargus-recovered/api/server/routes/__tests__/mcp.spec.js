...
const { getBasePath } = require('@askargus/api');
...
jest.mock('@askargus/api', () => {
  const actual = jest.requireActual('@askargus/api');
  return {
    ...actual,
    MCPOAuthHandler: {
    ...
    // Error handling utilities (from @askargus/api mcp/errors)
...
describe('MCP Routes', () => {
  ...
  describe('GET /:serverName/oauth/initiate', () => {
    const { MCPOAuthHandler } = require('@askargus/api');
  ...
  describe('GET /:serverName/oauth/callback', () => {
    const { MCPOAuthHandler, MCPTokenStorage } = require('@askargus/api');
    const { getLogStores } = require('~/cache');
  ...
  describe('POST /:serverName/reinitialize', () => {
    ...
    it('should handle server with custom user variables', async () => {
      ...
      require('~/config').getFlowStateManager.mockReturnValue({});
      require('~/cache').getLogStores.mockReturnValue({});
      require('@askargus/api').getUserMCPAuthMap.mockResolvedValue({
        'mcp:test-server': {
      ...
      expect(require('@askargus/api').getUserMCPAuthMap).toHaveBeenCalledWith({
        userId: 'test-user-id',
        servers: ['test-server'],

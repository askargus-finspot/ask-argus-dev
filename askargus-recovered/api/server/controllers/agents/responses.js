...
/** @type {import('@askargus/api').AppConfig | null} */
let appConfig = null;

/**
 * Set the app config for the controller
 * @param {import('@askargus/api').AppConfig} config
 */
 ...
 * @param {import('@askargus/api').InputItem[]} input
 ...
 * @param {import('@askargus/api').Response} response
 * @param {string} agentId
 * @returns {Promise<void>}
 */
...
const createResponse = async (req, res) => {
  ...
  try {
    ...
    if (actuallyStreaming) {
      ...
      /** @type {Promise<import('askargus-data-provider').TAttachment | null>[]} */
      const artifactPromises = [];
      // Use Responses API-specific callback that emits askargus:attachment events
...
const listModels = async (req, res) => {
  try {
    ...
    const models = agents.map((agent) => ({
      ...
      owned_by: agent.author ?? 'askargus',
      // Additional metadata
      name: agent.name,
 ...
 * The response ID maps to a conversationId in AskArgus's storage.
 *
 * @param {import('express').Request} req
 * @param {import('express').Response} res
 */

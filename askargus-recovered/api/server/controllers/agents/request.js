...
const { Constants, ViolationTypes } = require('askargus-data-provider');
const {
  sendEvent,
  getViolationInfo,
  buildMessageFiles,
...
function mergeSavedMessageFields(message, savedMessage) {
  if (!savedMessage) {
    return message;
  }
  ...
  if (mergedMessage.tokenCount == null && savedMessage.tokenCount != null) {
    mergedMessage.tokenCount = savedMessage.tokenCount;
  }

  if (mergedMessage.promptTokens == null && savedMessage.promptTokens != null) {
    mergedMessage.promptTokens = savedMessage.promptTokens;
  }

  if (mergedMessage.summaryTokenCount == null && savedMessage.summaryTokenCount != null) {
    mergedMessage.summaryTokenCount = savedMessage.summaryTokenCount;
  }

  if (mergedMessage.totalTokens == null && savedMessage.totalTokens != null) {
    mergedMessage.totalTokens = savedMessage.totalTokens;
  }

  if (mergedMessage.reasoningTokens == null && savedMessage.reasoningTokens != null) {
    mergedMessage.reasoningTokens = savedMessage.reasoningTokens;
  }

  if (mergedMessage.createdAt == null && savedMessage.createdAt != null) {
    mergedMessage.createdAt = savedMessage.createdAt;
  }

  if (mergedMessage.updatedAt == null && savedMessage.updatedAt != null) {
    mergedMessage.updatedAt = savedMessage.updatedAt;
  }
  ...
  try {
    ...
    const startGeneration = async () => {
      ...
      try {
        ...
        const { message: savedResponseMessage, conversation: convoData = {} } =
        ...
        let finalResponse = mergeSavedMessageFields(response, savedResponseMessage);
        ...
        if (client.savedMessageIds && !client.savedMessageIds.has(messageId)) {
          const savedFallbackResponse = await saveMessage(
            reqCtx,
            { ...response, user: userId, unfinished: wasAbortedBeforeComplete },
            { context: 'api/server/controllers/agents/request.js - resumable response end' },
          );
          finalResponse = mergeSavedMessageFields(finalResponse, savedFallbackResponse);
        }
        ...
        if (!wasAbortedBeforeComplete) {
          const finalEvent = {
            ...
            requestMessage: sanitizeMessageForTransmit(savedUserMessage),
          ...
          const finalEvent = {
            ...
            title: conversation.title,
            requestMessage: sanitizeMessageForTransmit(savedUserMessage),
            responseMessage: { ...finalResponse, unfinished: true },
          };
...
const _LegacyAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    const { message: savedResponseMessage, conversation: convoData = {} } = await databasePromise;
    const conversation = { ...convoData };
    conversation.title =
      conversation && !conversation.title ? null : conversation?.title || 'New Chat';
    ...
    if (!job.abortController.signal.aborted) {
      if (client.savedMessageIds && !client.savedMessageIds.has(messageId)) {
        const savedFinalResponse = await saveMessage(
          reqCtx,
          { ...finalResponse, user: userId },
          { context: 'api/server/controllers/agents/request.js - response end' },
        );
        finalResponse = mergeSavedMessageFields(finalResponse, savedFinalResponse);
      }
    ...
    // We need to ensure a final event is sent
      ...
      finalResponse = { ...finalResponse, error: true };

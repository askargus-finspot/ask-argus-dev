...
const { EModelEndpoint, Constants, openAISettings } = require('askargus-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing AskArgus conversation');
    return importAskArgusConvo;
  }
...
/**
 * Imports a AskArgus conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importAskArgusConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
  userRole,
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid AskArgus file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from AskArgus file`, error);
  }
}

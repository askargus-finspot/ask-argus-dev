...
function getReasoningTokensFromUsage(usage) {
  if (!usage || typeof usage !== 'object') {
    return undefined;
  }

  if (Number.isFinite(usage.reasoning_tokens) && usage.reasoning_tokens > 0) {
    return usage.reasoning_tokens;
  }

  const outputDetailsReasoningTokens = usage.output_tokens_details?.reasoning_tokens;
  if (Number.isFinite(outputDetailsReasoningTokens) && outputDetailsReasoningTokens > 0) {
    return outputDetailsReasoningTokens;
  }

  const completionDetailsReasoningTokens = usage.completion_tokens_details?.reasoning_tokens;
  if (Number.isFinite(completionDetailsReasoningTokens) && completionDetailsReasoningTokens > 0) {
    return completionDetailsReasoningTokens;
  }
...
class BaseClient {
  constructor(apiKey, options = {}) {
    ...
    /** @type {import('askargus-data-provider').VisionModes | undefined} */
    this.visionMode;
    /** @type {import('askargus-data-provider').FileConfig | undefined} */
    this._mergedFileConfig;
    /** @type {import('askargus-data-provider').EndpointFileConfig | undefined} */
  ...
  async sendMessage(message, opts = {}) {
    ...
    if (tokenCountMap && this.recordTokenUsage && this.getTokenCountForResponse) {
      ...
      const reasoningTokens = getReasoningTokensFromUsage(usage);
      responseMessage.totalTokens = promptTokens + completionTokens;
      if (reasoningTokens != null) {
        responseMessage.reasoningTokens = reasoningTokens;
      }
    }

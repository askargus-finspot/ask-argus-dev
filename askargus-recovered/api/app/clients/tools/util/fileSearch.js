...
const { generateShortLivedToken } = require('@askargus/api');
const { Tools, EToolResources } = require('askargus-data-provider');
...
const createFileSearchTool = async ({ userId, files, entity_id, fileCitations = false }) => {
  return tool(
    async ({ query }) => {
      ...
      /**
       * @param {import('askargus-data-provider').TFile} file

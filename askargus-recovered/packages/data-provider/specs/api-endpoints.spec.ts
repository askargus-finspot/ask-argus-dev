...
describe('buildLoginRedirectUrl', () => {
  ...
  it('returns plain /login for basename-prefixed /login (e.g. /askargus/login)', () => {
    window.history.replaceState({}, '', '/askargus/login?redirect_to=%2Fc%2Fabc');
    const result = buildLoginRedirectUrl();
    expect(result).toBe('/login');
  });

  it('returns plain /login for basename-prefixed /login sub-paths', () => {
    const result = buildLoginRedirectUrl('/askargus/login/2fa', '', '');
    expect(result).toBe('/login');
  });

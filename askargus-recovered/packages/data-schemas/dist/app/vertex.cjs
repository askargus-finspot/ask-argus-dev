...
function validateVertexConfig(vertexConfig) {
    ...
    const projectId = vertexConfig.projectId ? askargusDataProvider.extractEnvVariable(vertexConfig.projectId) : undefined;
    const region = askargusDataProvider.extractEnvVariable(vertexConfig.region || 'us-east5');
    const serviceKeyFile = vertexConfig.serviceKeyFile
        ? askargusDataProvider.extractEnvVariable(vertexConfig.serviceKeyFile)
        : undefined;
    const defaultDeploymentName = vertexConfig.deploymentName
        ? askargusDataProvider.extractEnvVariable(vertexConfig.deploymentName)
        : undefined;
    // Check for unresolved environment variables
    if (projectId && askargusDataProvider.envVarRegex.test(projectId)) {
        errors.push(`Vertex AI projectId environment variable "${vertexConfig.projectId}" was not found.`);
    }
    if (askargusDataProvider.envVarRegex.test(region)) {
        errors.push(`Vertex AI region environment variable "${vertexConfig.region}" was not found.`);
    }
    if (serviceKeyFile && askargusDataProvider.envVarRegex.test(serviceKeyFile)) {
        errors.push(`Vertex AI serviceKeyFile environment variable "${vertexConfig.serviceKeyFile}" was not found.`);
    }
    if (defaultDeploymentName && askargusDataProvider.envVarRegex.test(defaultDeploymentName)) {
...
function vertexConfigSetup(config) {
    var _a, _b;
    const anthropicConfig = (_a = config.endpoints) === null || _a === void 0 ? void 0 : _a[askargusDataProvider.EModelEndpoint.anthropic];
    ...
    winston.info('Vertex AI configuration loaded successfully', {
...
//# sourceMappingURL=vertex.cjs.map

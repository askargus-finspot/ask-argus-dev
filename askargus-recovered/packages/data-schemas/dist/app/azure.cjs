...
function azureConfigSetup(config) {
    var _a, _b, _c;
    const azureConfig = (_a = config.endpoints) === null || _a === void 0 ? void 0 : _a[askargusDataProvider.EModelEndpoint.azureOpenAI];
    if (!azureConfig) {
        throw new Error('Azure OpenAI configuration is missing.');
    }
    const { groups, ...azureConfiguration } = azureConfig;
    const { isValid, modelNames, modelGroupMap, groupMap, errors } = askargusDataProvider.validateAzureGroups(groups);
    if (!isValid) {
    ...
    for (const modelName of modelNames) {
        askargusDataProvider.mapModelToAzureConfig({ modelName, modelGroupMap, groupMap });
    ...
    if (azureConfiguration.assistants &&
        ...
        !process.env.ENDPOINTS.includes(askargusDataProvider.EModelEndpoint.azureAssistants)) {
        winston.warn(`Azure Assistants are configured, but the endpoint will not be accessible as it's not included in the ENDPOINTS environment variable.
      Please add the value "${askargusDataProvider.EModelEndpoint.azureAssistants}" to the ENDPOINTS list if expected.`);
    }
    return {
        errors,
        isValid,
...
exports.azureConfigSetup = azureConfigSetup;

 ...
 * Priority:
 * 1. ASKARGUS_LOG_DIR environment variable
 * 2. If running within AskArgus monorepo (when cwd ends with /api), use api/logs
...
const getLogDirectory = () => {
    if (process.env.ASKARGUS_LOG_DIR) {
        return process.env.ASKARGUS_LOG_DIR;
    }
    ...
    // For AskArgus project structure, use api/logs
    // For external consumers, they should set ASKARGUS_LOG_DIR
    if (cwd.includes('AskArgus')) {
        return apiLogsPath;
    }

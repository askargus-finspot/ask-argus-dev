 ...
 */
async function dropSupersededTenantIndexes(connection) {
    ...
    for (const [collectionName, indexNames] of Object.entries(SUPERSEDED_INDEXES)) {
        ...
        try {
            ...
            result.skipped.push(...indexNames.map((idx) => `${collectionName}.${idx} (collection does not exist)`));
            continue;
        }
        ...
        for (const indexName of indexNames) {
            ...
            try {
                ...
                winston.info(`[TenantMigration] Dropped superseded index: ${collectionName}.${indexName}`);
            }
            catch (err) {
                const msg = `${collectionName}.${indexName}: ${err.message}`;
                result.errors.push(msg);
                winston.error(`[TenantMigration] Failed to drop index: ${msg}`);
            }
        }
    }
    if (result.dropped.length > 0) {
        winston.info(`[TenantMigration] Migration complete. Dropped ${result.dropped.length} superseded indexes.`);
    }
    else {
        winston.info('[TenantMigration] No superseded indexes found — database is already migrated or fresh.');
    }
    return result;
}

exports.SUPERSEDED_INDEXES = SUPERSEDED_INDEXES;
exports.dropSupersededTenantIndexes = dropSupersededTenantIndexes;
//# sourceMappingURL=tenantIndexes.cjs.map

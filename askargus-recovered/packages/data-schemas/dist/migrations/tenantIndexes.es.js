...
async function dropSupersededTenantIndexes(connection) {
    ...
    if (result.dropped.length > 0) {
        logger.info(`[TenantMigration] Migration complete. Dropped ${result.dropped.length} superseded indexes.`);
    }
    else {
        logger.info('[TenantMigration] No superseded indexes found — database is already migrated or fresh.');
    }
    return result;
}

export { SUPERSEDED_INDEXES, dropSupersededTenantIndexes };
//# sourceMappingURL=tenantIndexes.es.js.map

...
async function tenantSafeBulkWrite(model, ops, options) {
    ...
    const sanitized = ops.map(sanitizeBulkOp).filter((op) => op != null);
    if (!tenantId) {
        if (isStrict()) {
...
const MUTATION_OPS = ['$set', '$unset', '$setOnInsert', '$rename'];
/**
 ...
 */
function stripTenantIdFromUpdate(update) {
    let result = null;
    ...
    for (const op of MUTATION_OPS) {
        const source = result !== null && result !== void 0 ? result : update;
        const payload = source[op];
    ...
    return result !== null && result !== void 0 ? result : update;
}

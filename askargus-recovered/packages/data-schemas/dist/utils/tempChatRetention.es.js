...
function getTempChatRetentionHours(interfaceConfig) {
    ...
    if (process.env.TEMP_CHAT_RETENTION_HOURS) {
        ...
        if (!isNaN(envValue)) {
            ...
            logger.warn(`Invalid TEMP_CHAT_RETENTION_HOURS environment variable: ${process.env.TEMP_CHAT_RETENTION_HOURS}. Using default: ${DEFAULT_RETENTION_HOURS} hours.`);
        }
    }
    ...
    if ((interfaceConfig === null || interfaceConfig === void 0 ? void 0 : interfaceConfig.temporaryChatRetention) !== undefined) {
        ...
        if (typeof configValue === 'number' && !isNaN(configValue)) {
            ...
            logger.warn(`Invalid temporaryChatRetention in config: ${configValue}. Using ${retentionHours} hours.`);
        }
    }
    // Validate the retention period
    if (retentionHours < MIN_RETENTION_HOURS) {
        logger.warn(`Temporary chat retention period ${retentionHours} is below minimum ${MIN_RETENTION_HOURS} hours. Using minimum value.`);
        retentionHours = MIN_RETENTION_HOURS;
    }
    else if (retentionHours > MAX_RETENTION_HOURS) {
        logger.warn(`Temporary chat retention period ${retentionHours} exceeds maximum ${MAX_RETENTION_HOURS} hours. Using maximum value.`);
...
export { DEFAULT_RETENTION_HOURS, MAX_RETENTION_HOURS, MIN_RETENTION_HOURS, createTempChatExpirationDate, getTempChatRetentionHours };
//# sourceMappingURL=tempChatRetention.es.js.map

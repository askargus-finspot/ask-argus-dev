...
function createSessionMethods(mongoose) {
    ...
    async function createSession(userId, options = {}) {
        ...
        try {
            ...
            winston.error('[createSession] Error creating session:', error);
    ...
    async function findSession(params, options = { lean: true }) {
        try {
            ...
            if (params.refreshToken) {
                const tokenHash = await index.hashToken(params.refreshToken);
            ...
            winston.error('[findSession] Error finding session:', error);
    ...
    async function updateExpiration(session, newExpiration, options = {}) {
        ...
        try {
            ...
            winston.error('[updateExpiration] Error updating session:', error);
    ...
    async function deleteSession(params) {
        try {
            ...
            const query = {};
            if (params.refreshToken) {
                query.refreshTokenHash = await index.hashToken(params.refreshToken);
            }
            if (params.sessionId) {
            ...
            if (result.deletedCount === 0) {
                winston.warn('[deleteSession] No session found to delete');
            }
            return result;
        }
        catch (error) {
            winston.error('[deleteSession] Error deleting session:', error);
    ...
    async function deleteAllUserSessions(userId, options = {}) {
        try {
            ...
            if (result.deletedCount && result.deletedCount > 0) {
                winston.debug(`[deleteAllUserSessions] Deleted ${result.deletedCount} sessions for user ${userIdString}.`);
            }
            return result;
        }
        catch (error) {
            winston.error('[deleteAllUserSessions] Error deleting user sessions:', error);
    ...
    async function generateRefreshToken(session) {
        ...
        try {
            ...
            const refreshToken = await index.signPayload({
                payload: {
                    id: session.user,
            ...
            session.refreshTokenHash = await index.hashToken(refreshToken);
            await session.save();
            return refreshToken;
        }
        catch (error) {
            winston.error('[generateRefreshToken] Error generating refresh token:', error);
    ...
    async function countActiveSessions(userId) {
        try {
            ...
            winston.error('[countActiveSessions] Error counting active sessions:', error);
...
exports.createSessionMethods = createSessionMethods;

...
function createAclEntryMethods(mongoose$1) {
    ...
    async function findEntriesByPrincipalsAndResource(principalsList, resourceType, resourceId) {
        ...
        const principalsQuery = principalsList.map((p) => ({
            ...
            ...(p.principalType !== askargusDataProvider.PrincipalType.PUBLIC && { principalId: p.principalId }),
        }));
    ...
    async function grantPermission(principalType, principalId, resourceType, resourceId, permBits, grantedBy, session, roleId) {
        ...
        if (principalType !== askargusDataProvider.PrincipalType.PUBLIC) {
            query.principalId =
                typeof principalId === 'string' && principalType !== askargusDataProvider.PrincipalType.ROLE
                    ? new mongoose.Types.ObjectId(principalId)
                    : principalId;
            if (principalType === askargusDataProvider.PrincipalType.USER) {
                query.principalModel = askargusDataProvider.PrincipalModel.USER;
            }
            else if (principalType === askargusDataProvider.PrincipalType.GROUP) {
                query.principalModel = askargusDataProvider.PrincipalModel.GROUP;
            }
            else if (principalType === askargusDataProvider.PrincipalType.ROLE) {
                query.principalModel = askargusDataProvider.PrincipalModel.ROLE;
            }
        }
        const update = {
            $set: {
     ...
     */
    async function deleteAclEntries(filter, options) {
        const AclEntry = mongoose$1.models.AclEntry;
     ...
     * @param options - Optional query options (e.g., { session })
     */
    async function bulkWriteAclEntries(ops, options) {
        const AclEntry = mongoose$1.models.AclEntry;
        return tenantBulkWrite.tenantSafeBulkWrite(AclEntry, ops, options || {});
    }
    /**
     ...
     */
    async function findPublicResourceIds(resourceType, requiredPermissions) {
        const AclEntry = mongoose$1.models.AclEntry;
        return AclEntry.find({
            principalType: askargusDataProvider.PrincipalType.PUBLIC,
            resourceType,
     ...
     */
    async function aggregateAclEntries(pipeline) {
        const AclEntry = mongoose$1.models.AclEntry;
     ...
     */
    async function getSoleOwnedResourceIds(userObjectId, resourceTypes) {
        const AclEntry = mongoose$1.models.AclEntry;
        const types = Array.isArray(resourceTypes) ? resourceTypes : [resourceTypes];
        const ownedEntries = await AclEntry.find({
            principalType: askargusDataProvider.PrincipalType.USER,
            principalId: userObjectId,
            resourceType: { $in: types },
            permBits: { $bitsAllSet: askargusDataProvider.PermissionBits.DELETE },
        })
            .select('resourceId')
            .lean();
        ...
        const otherOwners = await AclEntry.aggregate([
            {
                $match: {
                    ...
                    $or: [
                        ...
                        { principalType: { $ne: askargusDataProvider.PrincipalType.USER } },
                    ],
                },
            },
...
exports.createAclEntryMethods = createAclEntryMethods;

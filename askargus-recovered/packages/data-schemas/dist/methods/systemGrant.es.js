import { PrincipalType, SystemRoles } from 'askargus-data-provider';
import { CapabilityImplications, SystemCapabilities } from '../admin/capabilities.es.js';
import { tenantSafeBulkWrite } from '../utils/tenantBulkWrite.es.js';
import { normalizePrincipalId } from '../utils/principal.es.js';
...
const baseCapabilityValues = new Set(Object.values(SystemCapabilities));
/**
...
function createSystemGrantMethods(mongoose) {
    ...
    async function hasCapabilityForPrincipals({ principals, capability, tenantId, }) {
        ...
        const principalsQuery = principals
            .filter((p) => p.principalType !== PrincipalType.PUBLIC && p.principalId != null)
            .map((p) => ({
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    logger.warn(`[seedSystemGrants] Attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms: ${(_a = err.message) !== null && _a !== void 0 ? _a : String(err)}`);
                    await new Promise((resolve) => setTimeout(resolve, delay));
                }
                else {
                    logger.error('[seedSystemGrants] Failed to seed capabilities after all retries. ' +
...
export { createSystemGrantMethods };

...
function createKeyMethods(mongoose) {
    ...
    async function getUserKey(params) {
        ...
        if (!keyValue) {
            throw new Error(JSON.stringify({
                type: askargusDataProvider.ErrorTypes.NO_USER_KEY,
            }));
        }
        return await index.decrypt(keyValue.value);
    }
    /**
    ...
    async function getUserKeyValues(params) {
        ...
        try {
            ...
            winston.error('[getUserKeyValues]', e);
            throw new Error(JSON.stringify({
                type: askargusDataProvider.ErrorTypes.INVALID_USER_KEY,
            }));
        }
    }
    /**
    ...
    async function updateUserKey(params) {
        ...
        const Key = mongoose.models.Key;
        const encryptedValue = await index.encrypt(value);
        const updateObject = {
            userId,
            name,
...
exports.createKeyMethods = createKeyMethods;

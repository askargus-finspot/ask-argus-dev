        ...
        for (let attempt = 0; attempt < MAX_CREATE_RETRIES; attempt++) {
            try {
                ...
                if (isDuplicateKeyError(error) && attempt < MAX_CREATE_RETRIES - 1) {
                    ...
                    winston.debug(`[createMCPServer] Duplicate serverName detected, retrying (attempt ${attempt + 2}/${MAX_CREATE_RETRIES}) after ${delay}ms`);
    ...
    async function getListMCPServersByIds({ ids = [], otherParams = {}, limit = null, after = null, }) {
        ...
        if (after) {
            try {
                ...
                winston.warn('[getListMCPServersByIds] Invalid cursor provided', error);
            }
        }
...
exports.createMCPServerMethods = createMCPServerMethods;

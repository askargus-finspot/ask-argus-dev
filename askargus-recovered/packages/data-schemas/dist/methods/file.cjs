...
function createFileMethods(mongoose) {
    ...
    async function getToolFilesByIds(fileIds, toolResourceSet) {
        ...
        try {
            const orConditions = [];
            if (toolResourceSet.has(askargusDataProvider.EToolResources.context)) {
                orConditions.push({ text: { $exists: true, $ne: null }, context: askargusDataProvider.FileContext.agents });
            }
            if (toolResourceSet.has(askargusDataProvider.EToolResources.file_search)) {
            ...
            const filter = {
                file_id: { $in: fileIds },
                context: { $ne: askargusDataProvider.FileContext.execute_code },
                $or: orConditions,
            };
    ...
    async function getCodeGeneratedFiles(conversationId, messageIds) {
        ...
        try {
            const filter = {
                conversationId,
                context: askargusDataProvider.FileContext.execute_code,

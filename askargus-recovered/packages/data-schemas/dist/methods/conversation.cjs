...
function createConversationMethods(mongoose, messageMethods) {
    ...
    async function searchConversation(conversationId) {
        try {
            ...
            winston.error('[searchConversation] Error searching conversation', error);
    ...
    async function getConvo(user, conversationId) {
        try {
            ...
            winston.error('[getConvo] Error getting single conversation', error);
    ...
    async function deleteNullOrEmptyConversations() {
        try {
            ...
            winston.info(`[deleteNullOrEmptyConversations] Deleted ${result.deletedCount} conversations and ${messageDeleteResult.deletedCount} messages`);
            return {
            ...
            winston.error('[deleteNullOrEmptyConversations] Error deleting conversations', error);
    ...
    async function getConvoFiles(conversationId) {
        ...
        try {
            ...
            winston.error('[getConvoFiles] Error getting conversation files', error);
    ...
    async function saveConvo({ userId, isTemporary, interfaceConfig, }, { conversationId, newConversationId, ...convo }, metadata) {
        try {
            ...
            if (metadata === null || metadata === void 0 ? void 0 : metadata.context) {
                winston.debug(`[saveConvo] ${metadata.context}`);
            }
            ...
            if (isTemporary) {
                try {
                    ...
                    winston.info(`---\`saveConvo\` context: ${metadata === null || metadata === void 0 ? void 0 : metadata.context}`);
            ...
            if (!conversation) {
                winston.debug('[saveConvo] Conversation not found, skipping update');
                return null;
            }
            return conversation.toObject();
        }
        catch (error) {
            winston.error('[saveConvo] Error saving conversation', error);
            if (metadata === null || metadata === void 0 ? void 0 : metadata.context) {
                winston.info(`[saveConvo] ${metadata.context}`);
            }
    ...
    async function bulkSaveConvos(conversations) {
        try {
            ...
            const result = await tenantBulkWrite.tenantSafeBulkWrite(Conversation, bulkOps);
            return result;
        }
        catch (error) {
            winston.error('[bulkSaveConvos] Error saving conversations in bulk', error);
    ...
    async function getConvosByCursor(user, { cursor, limit = 25, isArchived = false, tags, search, sortBy = 'updatedAt', sortDirection = 'desc', } = {}) {
        ...
        if (search) {
            try {
                ...
                winston.error('[getConvosByCursor] Error during meiliSearch', error);
        ...
        if (cursor) {
            try {
                ...
                winston.warn('[getConvosByCursor] Invalid cursor format, starting from beginning');
            }
            if (cursorFilter) {
        ...
        try {
            ...
            winston.error('[getConvosByCursor] Error getting conversations', error);
    ...
    async function getConvosQueried(user, convoIds, cursor = null, limit = 25) {
        try {
            ...
            winston.error('[getConvosQueried] Error getting conversations', error);
    ...
    async function getConvoTitle(user, conversationId) {
        try {
            ...
            winston.error('[getConvoTitle] Error getting conversation title', error);
    ...
    async function deleteConvos(user, filter) {
        try {
            ...
            winston.error('[deleteConvos] Error deleting conversations and messages', error);
            throw error;
        }
    }
    return {
...
exports.createConversationMethods = createConversationMethods;

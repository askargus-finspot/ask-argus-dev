...
if (process.env.TENANT_ISOLATION_STRICT &&
    process.env.TENANT_ISOLATION_STRICT !== 'true' &&
    process.env.TENANT_ISOLATION_STRICT !== 'false') {
    winston.warn(`[TenantIsolation] TENANT_ISOLATION_STRICT="${process.env.TENANT_ISOLATION_STRICT}" ` +
        'is not "true" or "false"; defaulting to non-strict mode.');
}
const TENANT_ISOLATION_APPLIED = Symbol.for('askargus:tenantIsolation');
const VALUE_OPERATORS = ['$set', '$setOnInsert'];
const STRIP_OPERATORS = ['$unset', '$rename'];
/**
 ...
 */
function sanitizeTenantIdMutation(update) {
    if (!update) {
        return;
    }
...
function applyTenantIsolation(schema) {
    ...
    const queryMiddleware = function () {
        ...
        if (!tenantId || tenantId === tenantContext.SYSTEM_TENANT_ID) {
            return;
        }
        this.where({ tenantId });
    };
    const updateGuard = function () {
        const tenantId = tenantContext.getTenantId();
        if (tenantId === tenantContext.SYSTEM_TENANT_ID) {
            return;
        }
        sanitizeTenantIdMutation(this.getUpdate());
    ...
    schema.pre('aggregate', function () {
        const tenantId = tenantContext.getTenantId();
        if (!tenantId && isStrict()) {
            throw new Error('[TenantIsolation] Aggregate attempted without tenant context in strict mode');
        }
        if (!tenantId || tenantId === tenantContext.SYSTEM_TENANT_ID) {
            return;
        }
    ...
    schema.pre('insertMany', function (next, docs) {
        const tenantId = tenantContext.getTenantId();
        if (!tenantId && isStrict()) {
            return next(new Error('[TenantIsolation] insertMany attempted without tenant context in strict mode'));
        }
        if (tenantId && tenantId !== tenantContext.SYSTEM_TENANT_ID && Array.isArray(docs)) {
            for (const doc of docs) {
                if (!doc.tenantId) {
                    doc.tenantId = tenantId;
                }
                else if (isStrict() && doc.tenantId !== tenantId) {
                    return next(new Error('[TenantIsolation] Document tenantId does not match current tenant context'));
                }
            }
        }
        next();
...
//# sourceMappingURL=tenantIsolation.cjs.map

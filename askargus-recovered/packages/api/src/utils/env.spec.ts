...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      'User-Email': '{{ASKARGUS_USER_EMAIL}}',
      'User-Name': '{{ASKARGUS_USER_NAME}}',
      'User-Username': '{{ASKARGUS_USER_USERNAME}}',
      'User-Role': '{{ASKARGUS_USER_ROLE}}',
      'User-Id': '{{ASKARGUS_USER_ID}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{ASKARGUS_USER_EMAIL}}',
      'User-Username': '{{ASKARGUS_USER_USERNAME}}',
      'Non-Existent': '{{ASKARGUS_USER_NONEXISTENT}}',
    };
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      ASKARGUS_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{ASKARGUS_USER_EMAIL}}',
    };
  ...
  it('should handle multiple occurrences of the same placeholder', () => {
    ...
    const headers = {
      'Primary-Email': '{{ASKARGUS_USER_EMAIL}}',
      'Secondary-Email': '{{ASKARGUS_USER_EMAIL}}',
      'Backup-Email': '{{ASKARGUS_USER_EMAIL}}',
    };
  ...
  it('should handle mixed variable types in the same headers object', () => {
    ...
    const headers = {
      ...
      'X-User-Id': '{{ASKARGUS_USER_ID}}',
      'X-System-Key': '${TEST_API_KEY}',
      'X-User-Email': '{{ASKARGUS_USER_EMAIL}}',
      'Content-Type': 'application/json',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{ASKARGUS_USER_ID}}',
      'X-User-Name': '{{ASKARGUS_USER_NAME}}',
      'X-User-Username': '{{ASKARGUS_USER_USERNAME}}',
      'X-User-Email': '{{ASKARGUS_USER_EMAIL}}',
      'X-User-Provider': '{{ASKARGUS_USER_PROVIDER}}',
      'X-User-Role': '{{ASKARGUS_USER_ROLE}}',
      'X-User-GoogleId': '{{ASKARGUS_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{ASKARGUS_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{ASKARGUS_USER_OPENIDID}}',
      'X-User-SamlId': '{{ASKARGUS_USER_SAMLID}}',
      'X-User-LdapId': '{{ASKARGUS_USER_LDAPID}}',
      'X-User-GithubId': '{{ASKARGUS_USER_GITHUBID}}',
      'X-User-DiscordId': '{{ASKARGUS_USER_DISCORDID}}',
      'X-User-AppleId': '{{ASKARGUS_USER_APPLEID}}',
      'X-User-EmailVerified': '{{ASKARGUS_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{ASKARGUS_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{ASKARGUS_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{ASKARGUS_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{ASKARGUS_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{ASKARGUS_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{ASKARGUS_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders({ headers, user, customUserVars: customVars });
  ...
  it('should process ASKARGUS_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{ASKARGUS_BODY_CONVERSATIONID}}' };
    const result = resolveHeaders({ headers, body });
    expect(result['X-Conversation']).toBe('conv-123');
  });

  it('should not resolve env vars introduced via ASKARGUS_BODY placeholders', () => {
    const body = {
    ...
    const headers = {
      'X-Conv': '{{ASKARGUS_BODY_CONVERSATIONID}}',
      'X-Parent': '{{ASKARGUS_BODY_PARENTMESSAGEID}}',
      'X-Msg': '{{ASKARGUS_BODY_MESSAGEID}}',
    };
    const result = resolveHeaders({ headers, body });
  ...
  it('should not resolve env vars introduced via ASKARGUS_USER placeholders', () => {
    const user = createTestUser({ name: '${TEST_API_KEY}' });
    const headers = { 'X-Name': '{{ASKARGUS_USER_NAME}}' };
    const result = resolveHeaders({ headers, user });
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{ASKARGUS_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{ASKARGUS_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{ASKARGUS_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{ASKARGUS_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{ASKARGUS_BODY_CONVERSATIONID}}',
        parent: '{{ASKARGUS_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{ASKARGUS_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{ASKARGUS_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{ASKARGUS_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        USER_ID: '{{ASKARGUS_USER_ID}}',
        USER_EMAIL: '{{ASKARGUS_USER_EMAIL}}',
        USER_ROLE: '{{ASKARGUS_USER_ROLE}}',
      },
      args: ['--user', '{{ASKARGUS_USER_USERNAME}}', '--id', '{{ASKARGUS_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{ASKARGUS_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{ASKARGUS_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{ASKARGUS_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{ASKARGUS_USER_ID}} conversation:{{ASKARGUS_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{ASKARGUS_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{ASKARGUS_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{ASKARGUS_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{ASKARGUS_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
  ...
  describe('dbSourced flag', () => {
    ...
    it('should NOT resolve {{ASKARGUS_USER_*}} when dbSourced is true', () => {
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-User-Id']).toBe('{{ASKARGUS_USER_ID}}');
        expect(result.headers?.['X-User-Email']).toBe('{{ASKARGUS_USER_EMAIL}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{ASKARGUS_OPENID_*}} when dbSourced is true', () => {
      const user = {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          Authorization: 'Bearer {{ASKARGUS_OPENID_ACCESS_TOKEN}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.Authorization).toBe('Bearer {{ASKARGUS_OPENID_ACCESS_TOKEN}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });

    it('should NOT resolve {{ASKARGUS_BODY_*}} when dbSourced is true', () => {
      const body = {
        conversationId: 'conv-123',
        parentMessageId: 'parent-456',
        messageId: 'msg-789',
      ...
      const options: MCPOptions = {
        ...
        url: 'https://api.example.com',
        headers: {
          'X-Conversation': '{{ASKARGUS_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        expect(result.headers?.['X-Conversation']).toBe('{{ASKARGUS_BODY_CONVERSATIONID}}');
      } else {
        throw new Error('Expected streamable-http options');
      }
    });
    ...
    it('should resolve customUserVars but block all other placeholders when dbSourced is true', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'X-User-Id': '{{ASKARGUS_USER_ID}}',
          'X-Body': '{{ASKARGUS_BODY_CONVERSATIONID}}',
        },
      };
      ...
      if (isStreamableHTTPOptions(result)) {
        ...
        expect(result.headers?.['X-User-Id']).toBe('{{ASKARGUS_USER_ID}}');
        expect(result.headers?.['X-Body']).toBe('{{ASKARGUS_BODY_CONVERSATIONID}}');

import { logger } from '@askargus/data-schemas';
import type { Logger } from '@askargus/agents';
...
describe('getBasePath', () => {
  ...
  it('should return base path for subdirectory deployment', () => {
    ...
    expect(getBasePath()).toBe('/askargus');
  });
  ...
  it('should handle nested subdirectories', () => {
    ...
    expect(getBasePath()).toBe('/apps/askargus');
  });
  ...
  it('should handle URLs with query parameters', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/askargus?param=value';
    expect(getBasePath()).toBe('/askargus');
  });

  it('should handle URLs with fragments', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:3080/askargus#section';
    expect(getBasePath()).toBe('/askargus');
  });
  ...
  it('should handle URLs with ports', () => {
    process.env.DOMAIN_CLIENT = 'http://localhost:8080/askargus';
    expect(getBasePath()).toBe('/askargus');
  });

  it('should handle URLs with subdomains', () => {
    process.env.DOMAIN_CLIENT = 'https://app.example.com/askargus';
    expect(getBasePath()).toBe('/askargus');
  });
});

...
import type { IUser, OIDCTokens } from '@askargus/data-schemas';
 ...
 */
export const GRAPH_TOKEN_PLACEHOLDER = '{{ASKARGUS_GRAPH_ACCESS_TOKEN}}';
...
export function processOpenIDPlaceholders(
  ...
  for (const field of OPENID_TOKEN_FIELDS) {
    const placeholder = `{{ASKARGUS_OPENID_${field}}}`;
  ...
  const genericPlaceholder = '{{ASKARGUS_OPENID_TOKEN}}';

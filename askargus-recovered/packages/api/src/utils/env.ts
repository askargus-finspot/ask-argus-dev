import { extractEnvVariable } from 'askargus-data-provider';
 ...
 *
 * NOTE: This is a AskArgus-specific encoding scheme to work around Fetch API limitations.
...
function processUserPlaceholders(
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{ASKARGUS_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{ASKARGUS_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  ...
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{ASKARGUS_BODY_${field.toUpperCase()}}}`;

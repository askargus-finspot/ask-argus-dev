import { loadDefaultInterface } from '@askargus/data-schemas';
import { SystemRoles, Permissions, PermissionTypes, roleDefaults } from 'askargus-data-provider';
import type { TConfigDefaults, TCustomConfig } from 'askargus-data-provider';
...
describe('updateInterfacePermissions - permissions', () => {
  ...
  it('should preserve existing SHARE/SHARE_PUBLIC values when using boolean config (regression test)', async () => {
    ...
    // Config uses boolean (not object), simulating `agents: true` in askargus.yaml
    const config = {
      interface: {

import { logger, tenantStorage, SYSTEM_TENANT_ID } from '@askargus/data-schemas';
import {
  SystemRoles,
  Permissions,
  roleDefaults,
...
export async function updateInterfacePermissions({
  ...
  // 1. Explicit user configuration (from askargus.yaml)
  ...
  for (const roleName of [SystemRoles.USER, SystemRoles.ADMIN]) {
     ...
     * unless explicitly configured otherwise in askargus.yaml.
     */

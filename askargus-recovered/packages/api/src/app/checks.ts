import { logger, webSearchKeys } from '@askargus/data-schemas';
import { Constants, extractVariableName } from 'askargus-data-provider';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `askargus.yaml` config file instead.\nMore info: https://askargus.ai/docs/configuration/askargus_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.askargus.ai/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.askargus.ai/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://www.askargus.ai/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your askargus.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.askargus.ai/docs/configuration/askargus_yaml/web_search`,
        );
      }
    }
  });

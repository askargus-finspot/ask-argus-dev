 ...
 * 3. DB-sourced servers do NOT resolve `{{ASKARGUS_USER_*}}` placeholders.
 * 4. DB-sourced servers do NOT resolve `{{ASKARGUS_BODY_*}}` placeholders.
...
jest.mock('@askargus/data-schemas', () => ({
  logger: {
    info: jest.fn(),
    warn: jest.fn(),
    error: jest.fn(),
...
describe('dbSourced header security – integration', () => {
  ...
  it('DB-sourced: does NOT resolve {{ASKARGUS_USER_*}} placeholders', async () => {
    ...
    const captured = server.getLastHeaders();
    expect(captured['x-user-id']).toBe('{{ASKARGUS_USER_ID}}');
    expect(captured['x-user-email']).toBe('{{ASKARGUS_USER_EMAIL}}');
    expect(captured['x-user-id']).not.toBe('user-secret-id');
  });

  it('DB-sourced: does NOT resolve {{ASKARGUS_BODY_*}} placeholders', async () => {
    const body = {
    ...
    const captured = server.getLastHeaders();
    expect(captured['x-conv-id']).toBe('{{ASKARGUS_BODY_CONVERSATIONID}}');

...
describe('ServerConfigsCacheInMemory Integration Tests', () => {
  ...
  describe('credential placeholders in YAML configs', () => {
    it('should preserve ASKARGUS_OPENID placeholders (admin configs are trusted)', async () => {
      const adminConfig: ParsedServerConfig & { headers?: Record<string, string> } = {
        ...
        headers: {
          ...
          'X-User-Id': '{{ASKARGUS_OPENID_USER_ID}}',
        },
        updatedAt: FIXED_TIME,
      };
      ...
      expect(retrievedWithHeaders?.headers?.Authorization).toBe(
        'Bearer {{ASKARGUS_OPENID_ACCESS_TOKEN}}',
      );
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{ASKARGUS_OPENID_USER_ID}}');
    });

    it('should preserve ASKARGUS_USER placeholders (admin configs are trusted)', async () => {
      const adminConfig: ParsedServerConfig & { headers?: Record<string, string> } = {
        type: 'sse',
      ...
      expect(retrievedWithHeaders?.headers?.['X-User-Email']).toBe('{{ASKARGUS_USER_EMAIL}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Name']).toBe('{{ASKARGUS_USER_NAME}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{ASKARGUS_USER_ID}}');
    });
  });
});

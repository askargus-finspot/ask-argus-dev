# DNS Configuration for AskArgus Helm Chart

This feature allows you to configure custom DNS settings for AskArgus pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install askargus ./helm/askargus -f values.yaml
```
...
```bash
helm install askargus ./helm/askargus \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing AskArgus configuration
...
- Kubernetes 1.19+
- Compatible with all AskArgus deployment modes

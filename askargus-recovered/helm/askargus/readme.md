# AskArgus Helm Chart
This Askargus Helm Chart provides an easy, light weight template to deploy AskArgus on Kubernetes

## Variables
In this Chart, AskArgus will only work with environment Variables. You can Specify Vars and Secret using an existing Secret (This can be generated by [creating an Env File and converting it to a Kubernetes Secret](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#-em-secret-em-) `--from-env-file`)  
...
kind: Secret
metadata:
  name: askargus-credentials-env
  namespace: <askargus-chart-namespace>
type: Opaque
stringData:
...
Dependant of the Model you want to use, [create Credentials in your provider](https://docs.askargus.ai/install/configuration/ai_setup.html) and add them to the Secret:
```yaml
apiVersion: v1
kind: Secret
. . . .

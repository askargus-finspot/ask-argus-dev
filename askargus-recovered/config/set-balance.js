...
const { getBalanceConfig } = require('@askargus/api');
const { User, Balance } = require('@askargus/data-schemas').createModels(mongoose);
...
(async () => {
  ...
  if (!balanceConfig?.enabled) {
    console.red('Error: Balance is not enabled. Use askargus.yaml to enable it');
    silentExit(1);
  }

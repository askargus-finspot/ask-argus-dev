...
NGINX_CONF="/tmp/askargus-subdir-test-nginx.conf"
NGINX_PID="/tmp/askargus-subdir-test-nginx.pid"
...
worker_processes 1;
pid /tmp/askargus-subdir-test-nginx.pid;
error_log /tmp/askargus-subdir-test-nginx-error.log warn;
...
http {
    access_log /tmp/askargus-subdir-test-nginx-access.log;
...
start() {
    ...
    echo ""
    echo "=== Logs ==="
    echo "  Access: /tmp/askargus-subdir-test-nginx-access.log"
    echo "  Error:  /tmp/askargus-subdir-test-nginx-error.log"
    echo ""
...
stop() {
    ...
    rm -f "$NGINX_CONF" /tmp/askargus-subdir-test-nginx-*.log

...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all AskArgus Docker images...');

    const imageName = singleCompose ? 'askargus_single' : 'askargus';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image askargus:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new AskArgus image...');
  ...
  console.green('Your AskArgus app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for AskArgus to assure a fully clean installation.",
  );
